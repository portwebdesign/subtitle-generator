        ...
        thread::spawn(move || {
            ...
            match download_ffmpeg(&bin_dir) {
                ...
                Err(e) => {
                    ...
                    s.log.push(format!("✗ ffmpeg error: {}", e));
                }
            }
            ...
            match download_whisper(&bin_dir) {
                ...
                Err(e) => {
                    ...
                    s.log.push(format!("✗ whisper error: {}", e));
                }
            }
            ...
            match download_file(MODEL_TINY_URL, model_dest.to_str().unwrap_or("")) {
                ...
                Err(e) => {
                    ...
                    s.log.push(format!("✗ model error: {}", e));
                }
            }
            ...
            if all_ok {
                ...
                s.log.push("⚠ Some downloads failed. Check your internet connection and try again.".to_string());
            }
        });
    }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Transition from setup → main automatically when done
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if self.model_path().exists() {
                    ...
                    ui.label(
                        egui::RichText::new("⚠ Model not found")
            ...
            ui.horizontal(|ui| {
                ...
                ui.add_enabled_ui(!processing && self.all_ready(), |ui| {
                    if ui
                        .add(egui::Button::new(egui::RichText::new("▶ Generate").strong()))
                        .clicked()
            ...
            ui.horizontal(|ui| {
                ...
                ui.add_enabled_ui(!processing && self.all_ready(), |ui| {
                    if ui
                        .add(egui::Button::new(egui::RichText::new("▶ Generate All").strong()))
                        .clicked()
            ...
            ui.horizontal(|ui| {
                ui.label(
                    egui::RichText::new("whisper.cpp + ffmpeg")
                        .size(10.0)
                        .color(egui::Color32::GRAY),
                );
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if self.all_ready() {
                        ui.label(
                            egui::RichText::new("✓ All components ready")
                                .size(10.0)
                                .color(egui::Color32::GREEN),
                        );
                    } else {
                        ui.label(
                            egui::RichText::new("⚠ Missing components")
                                .size(10.0)
...
fn process_file(
    ...
    *progress.lock().unwrap() = 0.1;
    ...
    cmd.args(["-y", "-i", video, "-ar", "16000", "-ac", "1", "-f", "wav",
    ...
    if !out.status.success() {
        ...
        let tail = &err[err.len().saturating_sub(300)..];
    ...
    *progress.lock().unwrap() = 0.3;
    ...
    cmd.args([
        ...
        "-l", lang,
        "--output-srt",
        "--output-file", out_base.to_str().unwrap_or(""),
    ]);
    #[cfg(windows)]
    cmd.creation_flags(0x08000000);
    ...
    *progress.lock().unwrap() = 0.9;
    ...
    if !out.status.success() {
        let err = String::from_utf8_lossy(&out.stderr);
        let tail = &err[err.len().saturating_sub(300)..];
        return Err(format!("whisper: {}", tail));
    }

    if srt.exists() {
        log.lock().unwrap().push(format!(
            "  Saved: {}",
            srt.file_name().unwrap_or_default().to_str().unwrap_or("")
        ));
    }
...
fn download_ffmpeg(bin_dir: &Path) -> Result<(), String> {
    ...
    let zip = tmp.to_str().unwrap_or("").replace('\'', "''");
    let dst = dest.to_str().unwrap_or("").replace('\'', "''");
    let script = format!(
        "Add-Type -Assembly System.IO.Compression.FileSystem; \
         $z = [System.IO.Compression.ZipFile]::OpenRead('{zip}'); \
         $e = $z.Entries | Where-Object {{ $_.Name -eq 'ffmpeg.exe' }} | Select-Object -First 1; \
         if ($e) {{ [System.IO.Compression.ZipFileExtensions]::ExtractToFile($e, '{dst}', $true) }}; \
         $z.Dispose(); \
...
fn download_whisper(bin_dir: &Path) -> Result<(), String> {
    ...
    let zip = tmp.to_str().unwrap_or("").replace('\'', "''");
    let dst = dest.to_str().unwrap_or("").replace('\'', "''");
    // whisper.cpp releases name the binary 'main.exe' (older) or 'whisper-cli.exe' (newer)
    let script = format!(
        "Add-Type -Assembly System.IO.Compression.FileSystem; \
         $z = [System.IO.Compression.ZipFile]::OpenRead('{zip}'); \
         $e = $z.Entries | Where-Object {{ $_.Name -match '^(main|whisper|whisper-cli)\\.exe$' }} | Select-Object -First 1; \
         if ($e) {{ [System.IO.Compression.ZipFileExtensions]::ExtractToFile($e, '{dst}', $true) }}; \
         $z.Dispose(); \
         Remove-Item '{zip}' -Force"
    );
    run_ps(&script)?;
    if dest.exists() { Ok(()) } else { Err("whisper-cli.exe not found after extraction".into()) }
}
